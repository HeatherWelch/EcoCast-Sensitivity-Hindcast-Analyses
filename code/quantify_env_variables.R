...
for(i in pos_list){ ##missing 152, and start of 2015 (missing 12/29, 12/30) #153:length(b), 172:length(b)
  ...
  for(iii in 1:length(lags)){
    ...
    if(lag_date>as.Date(b[1])){
      ...
      for(ii in 1:length(vars)){
        q=abs(var_list[[ii]]-var_list_lag[[ii]])
        r=cellStats(q,sum)/12936 # spatial sum
        s=cellStats(q,sd)/12936 # spatial standard deviation
        t=cellStats(q>.5,sum)/12936 # % cells where difference > .5
        u=cellStats(q>.25,sum)/12936 # % cells where difference > .25
...
write.csv(DF_complete,"/Volumes/SeaGate/EcoCast_HW/EcoCastGit_Sensitivity_Hindcast/EcoCast-Sensitivity-Hindcast-Analyses/analysis_DFs/lagged_envt_vars_unstandardized.csv")
...
png("/Volumes/SeaGate/EcoCast_HW/EcoCastGit_Sensitivity_Hindcast/EcoCast-Sensitivity-Hindcast-Analyses/analysis_DFs/lagged_analysis_raw_vars_unstandardized.png",width=1100,height=1100,units='px',pointsize=35)
grid.arrange(a,b,c,d,top=textGrob("Lagged raw variable sensitivity analysis, unstandardized",gp=gpar(fontsize=20)))
dev.off()
