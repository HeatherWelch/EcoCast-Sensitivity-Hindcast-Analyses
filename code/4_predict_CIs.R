...
predCIs_master <- function(get_date,envdir,moddir,outdir,path,final_path_list,sensitivitydir){

  ############ 1. Load functions

  predCIs<-function(predDate,spname,modrep,envdir,outdir,baseGrid,sensitivitydir,modtype="pa",maskCCS=FALSE,varnames=NA){
    ...
    # mean
    png(paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_mean.png',sep=''),width=960,height=1280,units='px',pointsize=20)
    ...
    box(); dev.off()
    writeRaster(meanplot,filename=paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_mean.grd',sep=''),overwrite=TRUE)
    ## low CI
    png(paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_lowCI.png',sep=''),width=960,height=1280,units='px',pointsize=20)
    par(mar=c(3,3,.5,.5),las=1,font=2)
    if (modtype=='pa'){
    ...
    map('worldHires',add=TRUE,col=grey(0.7),fill=TRUE)
    box(); dev.off()
    writeRaster(lowplot,filename=paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_lowCI.grd',sep=''),overwrite=TRUE)
    ## high CI
    png(paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_highCI.png',sep=''),width=960,height=1280,units='px',pointsize=20)
    par(mar=c(3,3,.5,.5),las=1,font=2)
    if (modtype=='pa'){
    ...
    map('worldHires',add=TRUE,col=grey(0.7),fill=TRUE)
    box(); dev.off()
    writeRaster(highplot,filename=paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_highCI.grd',sep=''),overwrite=TRUE)
    # standard error
    png(paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_se.png',sep=''),width=960,height=1280,units='px',pointsize=20)
    par(mar=c(3,3,.5,.5),las=1,font=2)
    if (modtype=='pa'){
    ...
    map('worldHires',add=TRUE,col=grey(0.7),fill=TRUE)
    box(); dev.off()
    writeRaster(seplot,filename=paste(outdir,spname,'_',modtype,'_',predDate,"_",final_path_list$name,'_se.grd',sep=''),overwrite=TRUE)
    print(paste(Sys.time(),predDate, 'plotted'))
    #    }
  }
   ...
   ############ 4. rename colunms to reflect every possible model variable naming convention
    ...
    for(i in 1:length(CIobjs)){
      ...
      print(paste("Calculating confidence intervals for ",spname,sep=""))
	    CIdir<-paste(outdir,spname,"/predCIs/",sensitivitydir,"/",sep='') ## change this to date. lets save all CIs in folder with same date
	    ...
	    modtype='pa'
	    predCIs(predDate=dates,spname=spname,modrep=modrep,envdir=envdir,outdir=CIdir,sensitivitydir=sensitivitydir,baseGrid=baseGrid) 
    }
}
